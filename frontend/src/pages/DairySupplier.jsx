...
export default function DairySupplier() {
  ...
  const resetForm = () => { setEditEntry(null); setDate(dateToday()); setQuantity(''); setNotes(''); };
  ...
  const startEdit = row => { setDate(row.date); setQuantity(String(row.quantityKg || '')); setNotes(row.notes || ''); setEditEntry(row.id); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  const columns = [
    { key: 'date', label: 'Date' },
    { key: 'milk', label: 'Milk (Kg)', render: row => row.status === 'Received' ? `${row.quantityKg} Kg` : '—' },
    ...
    { key: 'actions', label: 'Actions', render: row => row.id ? <div className="flex gap-2 whitespace-nowrap"><Button variant="secondary" onClick={() => startEdit(row)}>Edit</Button><Button variant="danger" onClick={() => remove(row.id)}>Delete</Button></div> : <span className="text-xs text-[#6ab86a]">No entry</span> }
  ];
  ...
  return <div className="space-y-5 sm:space-y-6">
    ...
    <div className="grid min-w-0 gap-6 lg:grid-cols-[1fr_360px]">
      <div className="min-w-0 space-y-3"><div className="text-lg font-bold">Daily milk record</div><Table rows={monthly.calendar} columns={columns} emptyMessage="No dates in this collection period." /></div>
